...
#if (OPTION_CANAPI_LIBRARY != 0)
    while ((opt = getopt_long(argc, (char * const *)argv, "b:vm:z:y:lLTp:h", long_options, NULL)) != -1) {
#else
    while ((opt = getopt_long(argc, (char * const *)argv, "b:vm:z:y:lLTj:h", long_options, NULL)) != -1) {
#endif
        switch (opt) {
